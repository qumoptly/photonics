...
function format_u(r, units) {
  ...
  else if (r == Number.POSITIVE_INFINITY)
    return "infinity";
  else if (r == Number.NEGATIVE_INFINITY)
    return "-infinity";
  else {
...
function fval(s, units) {
  ...
  for (var j = -18; j <= 15; j++) {
    ...
    var sp = s.substr(s.length - 1);
    if ((sp == prefix[j]) || ((j == -6) && (sp == 'u'))) {
      e = e + j;
      s = s.substr(0, s.length - 1);
    }
  }
...
function GetFieldValues(not_field, calculatorname) {
  ...
  for (var j = 0; j < inp.length; j++) {
    if (inp[j].name != not_field) {
      window[inp[j].name] = fval(inp[j].value, funits[inp[j].name]);
      // instead of: eval(inp[j].name + '=' + fval(inp[j].value, funits[inp[j].name]));
    }
  }
}
